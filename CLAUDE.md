...
## Tickets (panel de TI) — flujo tipo tablero, SLA y bitácora

El detalle de cada ticket vive en `app/ti/tickets/[id]/page.tsx` (vista tipo Jira). La lógica de dominio está centralizada en **`lib/tickets.ts`** — reúsala, no repliques constantes:

- **Estados (6):** `abierto`, `en_proceso`, `en_espera`, `resuelto`, `cerrado`, `reabierto`. Cada uno trae etiqueta, tono de insignia y banderas (`activo`, `cuentaResuelto`). `ESTADOS_ACTIVOS` excluye resueltos/cerrados; `esEstadoActivo()` y `metaEstado()` son los accesos de UI. El portal del empleado los colapsa a sus 4 estados amigables vía `lib/portal.ts`.
- **SLA:** objetivos por prioridad (respuesta/resolución, en horas) en la constante `SLA`. `evaluarRespuesta()` / `evaluarResolucion()` devuelven `{ ms, objetivoMs, semaforo, pendiente }`; el semáforo (`en_tiempo` | `por_vencer` | `incumplido` | `cumplido` | `pausado` | `na`) se pinta con `components/PildoraSla.tsx`. **`en_espera` pausa el reloj** (no cuenta como incumplido). Es wall-clock, no horario hábil.
- **Marcas de tiempo:** `tickets.primera_respuesta_at` se sella al salir por primera vez de un estado sin atender (o al primer comentario); `tickets.resuelto_at` al pasar a resuelto/cerrado y se limpia al reabrir. Las server actions (`app/ti/tickets/actions.ts`) lo gestionan solas.
- **Bitácora:** tabla `ticket_eventos` (`comentario` | `estado` | `asignacion` | `sistema`). Cada cambio de estado/asignación y cada comentario registra un evento vía el helper `registrarEvento`. Son **notas internas**: nunca se exponen en el portal del empleado.
- **Edición inline:** todas las entidades del panel (tickets, inventario, empleados, mantenimientos) se editan sin salir de la pantalla mediante `<details>` plegables; en tablas el formulario sale como popover (`.panel-editar`) para no deformar la fila. Cero JS de cliente extra.
- Formato de tiempos: `fechaHora()` y `duracion(ms)` en `lib/format.ts`.

    ...
    <>
      ...
      {respAviso && (
        <div className="banner-exito" style={{ marginBottom: 20 }}>
          ...
          <div>
            <strong>
              Responsiva generada ·{" "}
              {folioResponsiva(plantillaDefault(respAviso.plantilla).prefijoFolio, respAviso.num)}
            </strong>
            Quedó como borrador.{" "}
      ...
      <div className="metricas compacta inv-resumen">
        ...
        <div className="metrica">
          ...
          <div className="metrica-label">
            {cat.valor === "celular" || cat.valor === "linea" ? "Libres" : "Sin asignar"}
          </div>
        </div>
        <div className="metrica">
      ...
      <div className="toolbar">
        <form className="filtros" method="get">
          ...
          {hayFiltro && (
            <Link href={`/ti/inventario?cat=${cat.valor}`} className="boton-texto">
              Limpiar
            </Link>
          )}
        </form>
      ...
      {lista.length === 0 ? (
        <div className="vacio">
          <strong>
            {hayFiltro ? "Sin coincidencias" : `Sin registros en ${cat.etiqueta.toLowerCase()}`}
          </strong>
          {hayFiltro
        ...
        <table className="tabla inv-tabla">
          ...
          <tbody>
            {lista.map((e) => {
              ...
              const ins = r
                ? (ESTADOS_RESP[r.estado as EstadoResponsiva] ?? ESTADOS_RESP.borrador)
                : null;
              const porVencer =
                c.fechas &&
                e.garantia_hasta &&
                    ...
                    {e.asignado_email ? (
                      <span className="inv-asignado">
                        <span className="inv-avatar" aria-hidden>
                          {iniciales(e.asignado_a ?? e.asignado_email)}
                        </span>
                        <span>
                  ...
                  <td data-label="Resguardo">
                    {r && ins ? (
                      <Link
                        ...
                        className={`resguardo-chip ${ins.tono}`}
                          ...
                          <dl className="modal-datos">
                            ...
                            {c.fechas && (
                              <div>
                                ...
                                <dd className="mono">{fechaCorta(e.garantia_hasta)}</dd>
                              </div>
                            )}
                            {camposCat.map((def) => {

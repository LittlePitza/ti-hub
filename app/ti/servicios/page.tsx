...
export default async function EstadoSistemas() {
  ...
  const head = (
    <div className="pagina-head">
      <div>
        ...
        <p className="pagina-desc">Servicios que la empresa usa a diario y el registro de sus caídas</p>
      </div>
    </div>
  );
  ...
  const [serviciosQ, abiertosQ, historialQ] = await Promise.all([
    ...
    sb.from("incidentes").select("*").neq("estado", "resuelto").order("inicio", { ascending: false }),
    sb
      .from("incidentes")
  ...
  const conEstado = serviciosConEstado(servicios.filter((s) => s.activo), abiertos);
  ...
  return (
    <>
      ...
      <section className={`estado-hero ${resumen.todoBien ? "ok" : resumen.caidos > 0 ? "critico" : "aviso"}`}>
        <span className={`estado-punto grande ${resumen.todoBien ? "ok" : resumen.caidos > 0 ? "critico" : "aviso"}`} aria-hidden />
      ...
      <section className="seccion">
        ...
        {conEstado.length === 0 ? (
          <div className="vacio">
            ...
            Agrega los servicios que la empresa usa (internet, Microsoft, SAP…) en «Administrar servicios», más abajo.
          </div>
        ) : (
      ...
      <form className="formulario" action={registrarIncidente}>
        ...
        <div className="campos">
          ...
          <div className="campo ancho">
            ...
            <input id="inc-titulo" name="titulo" required placeholder="Sin internet en toda la planta" />
          </div>
          <div className="campo ancho">
            <label htmlFor="inc-desc">Detalle (opcional)</label>
            <textarea id="inc-desc" name="descripcion" placeholder="Qué se ve, a quién afecta, qué se está revisando…" />
          </div>
        </div>
        <BotonEnviar className="boton" ocupado="Registrando…">Registrar caída</BotonEnviar>
      </form>
      ...
      <section className="seccion">
        ...
        {historial.length === 0 ? (
          <div className="vacio">Aún no hay incidentes resueltos. Los que registres aparecerán aquí al cerrarlos.</div>
        ) : (
                ...
                return (
                  <tr key={i.id}>
                    ...
                    <td>
                      ...
                      {i.resolucion && <div className="suave" style={{ fontSize: 12.5 }}>{i.resolucion}</div>}
                    </td>
                    <td><span className={`insignia ${tipo.tono}`}>{tipo.etiqueta}</span></td>
                    <td className="mono">{duracion(duracionIncidente(i, ahora))}</td>
                    <td className="suave mono" style={{ whiteSpace: "nowrap" }}>{fechaHora(i.fin)}</td>
          ...
          <div style={{ marginTop: 14 }}>
            <form className="bloque-form" action={crearServicio} style={{ marginBottom: 18 }}>
              <div className="dos-col">
                <div>
                  <label className="mini-label" htmlFor="sv-nombre">Nombre</label>
                  <input id="sv-nombre" name="nombre" required placeholder="SAP, Internet, Microsoft 365…" />
                </div>
                <div>
                  <label className="mini-label" htmlFor="sv-proveedor">Proveedor (opcional)</label>
                  <input id="sv-proveedor" name="proveedor" placeholder="Telmex, Microsoft…" />
                </div>
              </div>
              <div className="dos-col">
                <div>
                  <label className="mini-label" htmlFor="sv-categoria">Categoría</label>
                  <select id="sv-categoria" name="categoria" defaultValue="plataforma">
                    {CATEGORIAS_SERVICIO.map((c) => <option key={c.valor} value={c.valor}>{c.etiqueta}</option>)}
                  </select>
                </div>
                <div>
                  <label className="mini-label" htmlFor="sv-criticidad">Criticidad</label>
                  <select id="sv-criticidad" name="criticidad" defaultValue="normal">
                    {CRITICIDADES.map((c) => <option key={c.valor} value={c.valor}>{c.etiqueta}</option>)}
                  </select>
                </div>
              </div>
              <label className="mini-label" htmlFor="sv-desc">Descripción (opcional)</label>
              ...
              <BotonEnviar className="boton mini" ocupado="Agregando…">Agregar servicio</BotonEnviar>
            </form>

            <table className="tabla">
              <thead>
                <tr><th>Servicio</th><th>Categoría</th><th>Criticidad</th><th>Portal</th><th></th></tr>
              </thead>
              <tbody>
                {servicios.map((s) => (
                  <tr key={s.id}>
                    <td>
                      <div className="celda-principal">{s.nombre}{!s.activo && <span className="insignia neutro" style={{ marginLeft: 8 }}>inactivo</span>}</div>
                      {s.proveedor && <div className="suave" style={{ fontSize: 12.5 }}>{s.proveedor}</div>}
                    </td>
                    <td className="suave">{CATEGORIAS_SERVICIO.find((c) => c.valor === s.categoria)?.etiqueta ?? s.categoria}</td>
                      ...
                      <div className="fila-acciones">
                        <details className="plegable interno editar">
                          ...
                          <form action={editarServicio} className="bloque-form panel-editar">
                            ...
                            <label className="check-linea">
                              <input type="checkbox" name="visible_portal" defaultChecked={s.visible_portal} />
                        ...
                        <form action={eliminarServicio}>
                          <input type="hidden" name="id" value={s.id} />
                          <BotonEnviar className="boton secundario mini" style={{ color: "var(--critico)" }} ocupado="…" title="Borra el servicio y su historial de incidentes">Eliminar</BotonEnviar>
                        </form>
                      </div>
                    </td>
                  </tr>
          ...
          <div className="incidente-linea" key={i.id}>
            <div className="incidente-linea-cuerpo">
              <div className="incidente-linea-cab">
                ...
                <span className="mono suave" style={{ fontSize: 12.5 }}>{folioIncidente(i.num)}</span>
                <span className={`insignia ${est.tono}`}>{est.etiqueta}</span>
              </div>
              <div className="incidente-linea-titulo">{i.titulo}</div>
              {i.descripcion && <div className="suave" style={{ fontSize: 13 }}>{i.descripcion}</div>}
              <div className="incidente-linea-meta">
                Desde {fechaHora(i.inicio)} · <b>{duracion(duracionIncidente(i, ahora))}</b> sin servicio
              </div>
            </div>
            <div className="incidente-linea-acciones">
              {i.estado === "activo" && (
                <form action={vigilarIncidente}>
                  <input type="hidden" name="id" value={i.id} />
                  <BotonEnviar className="boton secundario mini" ocupado="…" title="Ya se restableció pero sigues observando">Vigilando</BotonEnviar>
                </form>
              )}
              <details className="plegable interno editar">
                ...
                <form action={resolverIncidente} className="bloque-form panel-editar">
                  ...
                  <textarea name="resolucion" rows={2} placeholder="Qué se hizo para restablecerlo" />
                  <label className="mini-label">Terminó (opcional, por defecto ahora)</label>
                  <input name="fin" type="datetime-local" />
                  <BotonEnviar className="boton mini" ocupado="Resolviendo…">Marcar resuelto</BotonEnviar>
                </form>
              </details>
            </div>
          </div>

// Server-rendered charts in plain SVG/CSS. Colours come from the theme
// variables, so light and dark mode work with no changes.

export type ChartDatum = { label: string; value: number; tone: string };

const TONOS: Record<string, string> = {
  ok: "var(--ok)",
  aviso: "var(--aviso)",
  critico: "var(--critico)",
  info: "var(--petroleo)",
  neutro: "var(--tinta-suave)",
};

export function Donut({ datos, unit }: { datos: ChartDatum[]; unit?: string }) {
  const total = datos.reduce((s, d) => s + d.value, 0);
  if (total === 0) return <div className="grafica-vacia">Sin datos todavía</div>;

  const R = 40;
  const C = 2 * Math.PI * R;
  let recorrido = 0;
  const segmentos = datos
    .filter((d) => d.value > 0)
    .map((d) => {
      const largo = (d.value / total) * C;
      const seg = { ...d, largo, inicio: recorrido };
      recorrido += largo;
      return seg;
    });

  return (
    <div className="dona">
      <svg
        className="dona-svg"
        viewBox="0 0 100 100"
        role="img"
        aria-label={datos.map((d) => `${d.label}: ${d.value}`).join(", ")}
      >
        {segmentos.map((s) => (
          <circle
            key={s.label}
            cx="50"
            cy="50"
            r={R}
            fill="none"
            stroke={TONOS[s.tone] ?? TONOS.neutro}
            strokeWidth="13"
            strokeDasharray={`${s.largo} ${C - s.largo}`}
            strokeDashoffset={-s.inicio}
            transform="rotate(-90 50 50)"
          />
        ))}
        <text x="50" y={unit ? 49 : 54} textAnchor="middle" className="dona-total">
          {total}
        </text>
        {unit && (
          <text x="50" y="61" textAnchor="middle" className="dona-unidad">
            {unit}
          </text>
        )}
      </svg>
      <ul className="dona-leyenda">
        {datos.map((d) => (
          <li key={d.label}>
            <span className="punto" style={{ background: TONOS[d.tone] ?? TONOS.neutro }} />
            <span>{d.label}</span>
            <span className="mono">{d.value}</span>
          </li>
        ))}
      </ul>
    </div>
  );
}

// Monthly trend with two series side by side (for example tickets created vs
// resolved). Vertical columns because the X axis is time; labels sit underneath
// and the value on top of each column.
export type ColumnPoint = { label: string; a: number; b: number };

export function Columns({
  datos,
  serieA,
  serieB,
  tonoA = "info",
  tonoB = "ok",
}: {
  datos: ColumnPoint[];
  serieA: string;
  serieB: string;
  tonoA?: string;
  tonoB?: string;
}) {
  const max = Math.max(...datos.map((d) => Math.max(d.a, d.b)), 1);
  if (datos.every((d) => d.a === 0 && d.b === 0))
    return <div className="grafica-vacia">Sin datos todavía</div>;

  // Canvas geometry: a 150px-tall drawing area; each month's two columns sit
  // tight as a pair, centred in their cell.
  const ANCHO = 560;
  const ALTO = 196;
  const BASE = 158; // y of the baseline
  const ALTO_MAX = 128; // height of the tallest column
  const celda = ANCHO / datos.length;
  const barra = Math.min(20, celda / 3.2);

  const altura = (v: number) => (v === 0 ? 0 : Math.max((v / max) * ALTO_MAX, 3));
  const colorA = TONOS[tonoA] ?? TONOS.neutro;
  const colorB = TONOS[tonoB] ?? TONOS.neutro;

  return (
    <div className="columnas">
      <svg
        className="columnas-svg"
        viewBox={`0 0 ${ANCHO} ${ALTO}`}
        role="img"
        aria-label={datos.map((d) => `${d.label}: ${serieA} ${d.a}, ${serieB} ${d.b}`).join("; ")}
      >
        <line x1="0" y1={BASE} x2={ANCHO} y2={BASE} className="columnas-eje" />
        {datos.map((d, i) => {
          const cx = celda * i + celda / 2;
          const hA = altura(d.a);
          const hB = altura(d.b);
          return (
            <g key={d.label + i}>
              <rect
                x={cx - barra - 1.5}
                y={BASE - hA}
                width={barra}
                height={hA}
                rx="3"
                fill={colorA}
              />
              <rect x={cx + 1.5} y={BASE - hB} width={barra} height={hB} rx="3" fill={colorB} />
              <text
                x={cx - barra / 2 - 1.5}
                y={BASE - hA - 5}
                textAnchor="middle"
                className="columnas-valor"
              >
                {d.a}
              </text>
              <text
                x={cx + barra / 2 + 1.5}
                y={BASE - hB - 5}
                textAnchor="middle"
                className="columnas-valor"
              >
                {d.b}
              </text>
              <text x={cx} y={BASE + 20} textAnchor="middle" className="columnas-mes">
                {d.label}
              </text>
            </g>
          );
        })}
      </svg>
      <ul className="columnas-leyenda">
        <li>
          <span className="punto" style={{ background: colorA }} />
          <span>{serieA}</span>
        </li>
        <li>
          <span className="punto" style={{ background: colorB }} />
          <span>{serieB}</span>
        </li>
      </ul>
    </div>
  );
}

export function Bars({ datos }: { datos: ChartDatum[] }) {
  const total = datos.reduce((s, d) => s + d.value, 0);
  if (total === 0) return <div className="grafica-vacia">Sin datos todavía</div>;

  const max = Math.max(...datos.map((d) => d.value), 1);
  return (
    <div className="barras">
      {datos.map((d) => (
        <div className="barra-fila" key={d.label}>
          <span className="barra-label" title={d.label}>
            {d.label}
          </span>
          <div className="barra-pista">
            <div
              className="barra-relleno"
              style={{
                width: d.value === 0 ? 0 : `${Math.max((d.value / max) * 100, 4)}%`,
                background: TONOS[d.tone] ?? TONOS.neutro,
              }}
            />
          </div>
          <span className="barra-valor mono">{d.value}</span>
        </div>
      ))}
    </div>
  );
}

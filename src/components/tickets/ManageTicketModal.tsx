...
// Managing a device opens a Notion-style modal card: a small peek view with the
// summary and the most common action, plus a button to expand it into the large
// card with the full editor. Same native <dialog> (backdrop, focus trap, Escape);
// the mode (compact/expanded) only changes the width and which block is shown, so
// the summary and the editor coexist without remounting — nothing typed into the
// credentials is lost when toggling.

// Maximise/minimise icons (diagonal arrows), hoisted out of the render.
...
export default function ManageTicketModal({
  titulo,
  subtitulo,
  resumen,
  children,
  ...
  const abrir = () => {
    setExpandida(false); // always opens in the small peek view

...
import { createTicket, type EstadoCrear } from "@/app/ti/tickets/actions";
import SubmitButton from "@/components/ui/SubmitButton";
import RequesterPicker, { type EmployeeOption } from "@/components/shared/RequesterPicker";

// "Nuevo ticket" opens a modal (the same native <dialog> and styling as
// ManageTicketModal) with the create form. The requester is chosen from an
// employee combobox or typed by hand. useActionState carries the server action's
// result: on a successful create the modal closes and the form clears.
...
export default function NewTicketModal({ empleados }: { empleados: EmployeeOption[] }) {
  const ref = useRef<HTMLDialogElement>(null);
  const formRef = useRef<HTMLFormElement>(null);
  const [estado, accion] = useActionState<EstadoCrear, FormData>(createTicket, null);
  ...
  // On a successful create: clear the form and close. (The state is a fresh
  // reference per submission, so the effect runs on every success.)
  useEffect(() => {
    if (estado?.ok) {
          ...
          <div className="modal-gestionar-body">
            <form ref={formRef} action={accion} className="formulario plano">
              <div className="campos">
                <RequesterPicker empleados={empleados} onCorreo={setCorreo} />
              ...
              <SubmitButton className="boton" ocupado="Creando…">
                Crear ticket
              </SubmitButton>
            </form>
          </div>

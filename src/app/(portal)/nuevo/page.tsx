...
import { getPortalEmail, PORTAL_CATEGORIES } from "@/lib/domain/portal";
import ImagePicker from "@/components/shared/ImagePicker";
import SubmitButton from "@/components/ui/SubmitButton";
import ServicesNotice from "@/components/shared/ServicesNotice";
import { createPortalReport } from "../actions";
    ...
    <>
      ...
      {error && (
        <div className="login-error" style={{ marginBottom: 20 }}>
          {ERRORES[error] ?? ERRORES.guardar}
        </div>
      )}

      <form action={createPortalReport}>
        <section className="paso">
          <h2 className="paso-titulo">
            <span className="paso-num">1</span>¿Qué tipo de problema es?
          </h2>
          <div className="opciones">
            {PORTAL_CATEGORIES.map((c) => (
              <label className="opcion" key={c.value}>
                <input type="radio" name="categoria" value={c.value} required />
                <span className="opcion-cuerpo">
                  <span className="opcion-icono">{ICONO_CATEGORIA[c.value]}</span>
                  <span className="opcion-texto">
                    <span className="opcion-titulo">{c.titulo}</span>
                    <span className="opcion-detalle">{c.detail}</span>
                  </span>
                </span>
              </label>
            ))}
        ...
        {equipos.length > 0 && (
          <section className="paso">
            <h2 className="paso-titulo">
              <span className="paso-num">2</span>¿Con cuál de tus equipos?
            </h2>
            <div className="opciones">
              {equipos.map((e) => (
              ...
              <label className="opcion">
                ...
                <span className="opcion-cuerpo">
                  ...
                  <span className="opcion-texto">
                    ...
                    <span className="opcion-detalle">
                      No tiene que ver con un equipo de la lista
                    </span>
                  </span>
                </span>
              </label>
        ...
        <section className="paso">
          <h2 className="paso-titulo">
            <span className="paso-num">{pasoFinal}</span>Cuéntanos qué pasa
          </h2>
        ...
        <SubmitButton className="portal-boton" ocupado="Enviando tu reporte…">
          Enviar reporte
          <IconoFlecha />
        </SubmitButton>
...
// One icon per category, so the employee recognises their problem at a glance
// without reading.

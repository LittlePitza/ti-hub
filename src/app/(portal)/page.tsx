...
import ServicesNotice from "@/components/shared/ServicesNotice";
import {
  signInToPortal,
  signOutOfPortal,
  archivePortalReport,
...
export default async function Portal({
  ...
  const activos = tickets.filter((t) => PORTAL_STATUS[t.estado]?.step !== 3).length;
  // Archived reports are tucked into their own tab; the rest stays in view.
  const visibles = tickets.filter((t) => t.estado !== "archivado");
  const archivados = tickets.filter((t) => t.estado === "archivado");

  // How many IT replies each ticket has (only `respuesta` events). The full text
  // lives on the report detail page; the card only needs the count.
    ...
    <>
      ...
      {creado && Number.isFinite(Number(creado)) && (
        <div className="banner-exito" role="status">
          <IconoCheck />
          <div>
            <strong>Listo, recibimos tu reporte {ticketFolio(Number(creado))}.</strong>
      ...
      <section className="portal-hero">
        ...
        <div className="portal-hero-pie">
          ...
          <form action={signOutOfPortal}>
            <SubmitButton className="portal-hero-cambiar" ocupado="Saliendo…">
              No soy yo
            </SubmitButton>
          </form>
        </div>
              ...
              return (
                <article className="ticket-card" key={t.id}>
                  ...
                  <div className="ticket-card-pie">
                    ...
                    <form action={archivePortalReport} className="ticket-card-archivar">
                ...
                <li className="portal-archivado" key={t.id}>
                  <Link className="portal-archivado-cuerpo" href={`/reporte/${t.id}`}>
                    ...
                    <span className="portal-archivado-meta mono">
                      {ticketFolio(t.num)} · {shortDate(t.created_at)}
                    </span>
                  </Link>
                  <form action={reopenPortalReport}>
                    <input type="hidden" name="id" value={t.id} />
                    <SubmitButton
...
function Bienvenida({ conError }: { conError: boolean }) {
  // The same essence as the panel login and the Portal de Mantenimiento: a navy
  // background, a logo card with an eyebrow, and a white card holding the form.
  // It keeps the merged email field, the Continuar button and the password-less note.
  return (
    <div className="portal-acceso">
      <div className="login-col">
        ...
        <form className="login-caja" action={signInToPortal}>
          ...
          <SubmitButton className="portal-boton press" ocupado="Un momento…">
            Continuar
            <IconoFlecha />
          </SubmitButton>
...
// Waves/arrows from the PIMSA isotype as the hero watermark (circular economy).
function Onda() {
  return (
    <svg

...
// Saves the email-signature defaults (website, address and tagline) into the
// single config_correo row. They apply to every signature generated afterwards.
// `id` is the employee whose page the edit came from (so we can navigate back).
export async function saveSignatureSettings(formData: FormData) {
...
// Full employee edit. If the email changes, the linked devices are updated too
// (asignado_email) so the link to the portal is not broken.
export async function editEmployee(formData: FormData) {
  ...
  if (previo?.correo && previo.correo !== correoNuevo) {
    ...
    // Keep the denormalised name in the inventory up to date.
    const { error: errEq } = await sb
      .from("equipos")
...
export async function deleteEmployee(formData: FormData) {
  ...
  // Their devices are freed (ticket history is preserved by email).
  if (emp?.correo) {
    const { error: errEq } = await sb
      .from("equipos")

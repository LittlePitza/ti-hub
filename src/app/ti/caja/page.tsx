...
import { pettyCashSummary, pettyCashLevel, type PettyCashEntry } from "@/lib/domain/pettyCash";
...
export default async function CajaChica() {
  ...
  const hoy = todayISO();
  const movimientos = (movs ?? []) as PettyCashEntry[];
  const r = pettyCashSummary(movimientos, config?.caja_limite ?? null);
  const nivel = pettyCashLevel(r);

  // The limit form: reused by the unconfigured empty state and by the accordion.
  const formLimite = (
    <form action={saveFundLimit} className="bloque-form">
  ...
  const fila = (m: PettyCashEntry) => {
    const esCompra = m.tipo === "compra";
    return (
      <tr key={m.id}>
        <td className="mono">{shortDate(m.fecha)}</td>
        <td>
  ...
  return (
    <>
        ...
        <section className="caja-hero">
          <div className="caja-fondo">
            ...
            <div className="caja-saldo">
              {currency(r.saldo)}
              <span className="de-limite">de {currency(r.limite)}</span>
            </div>
            <div
              className="caja-barra"
              role="img"
              aria-label={`Queda ${currency(r.saldo)} de ${currency(r.limite)}`}
            >
              <div
                className={`caja-barra-fill ${nivel.tone}`}
                style={{ width: `${Math.round(nivel.pct * 100)}%` }}
              />
            </div>
            <div className="caja-extremos">
              <span>$0</span>
              <span>fondo {currency(r.limite)}</span>
            </div>
            <div className="caja-pie">
              Gastado desde el último reembolso: <b>{currency(r.gastadoDesdeReembolso)}</b>
              {r.ultimoReembolso && <> · Último reembolso: {shortDate(r.ultimoReembolso.fecha)}</>}
            </div>
          </div>
          <div className="caja-acciones">
            <details className="plegable">
              <summary>Registrar reembolso</summary>
              <form action={recordReimbursement} className="bloque-form">
                ...
                {r.porReembolsar > 0 && (
                  <p className="suave" style={{ fontSize: 12, margin: "2px 0 0" }}>
                    Faltan {currency(r.porReembolsar)} para volver al fondo completo.
                  </p>
                )}
                <div className="dos-col">
                  <div>
                ...
                />
                <SubmitButton className="boton mini" ocupado="Registrando…">
                  Registrar reembolso
                </SubmitButton>
              </form>
            </details>
      ...
      <form className="formulario" action={recordPurchase}>

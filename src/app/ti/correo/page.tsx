...
import { getEmailConfig, isEmailReady, hasCredentials } from "@/lib/domain/email";
import NoConnection from "@/components/ui/NoConnection";
import SubmitButton from "@/components/ui/SubmitButton";
import { saveEmailConfig, sendTestMessage, connectMicrosoft, disconnectMicrosoft } from "./actions";

export const dynamic = "force-dynamic";
export const metadata = { title: "Correo" };

const AVISOS_PRUEBA: Record<string, { tone: string; text: string }> = {
  ok: { tone: "ok", text: "Correo de prueba enviado. Revisa la bandeja (y el spam)." },
  error: {
    tone: "critico",
    text: "No se pudo enviar. Revisa el método de envío y las credenciales.",
  },
  sincreds: {
    tone: "aviso",
    text: "Faltan credenciales para el método elegido. Guárdalas antes de probar.",
  },
  falta: { tone: "aviso", text: "Escribe un correo de destino para la prueba." },
  conectado: { tone: "ok", text: "Cuenta de Microsoft conectada. Ya puedes enviar correos." },
  desconectado: { tone: "aviso", text: "Se desconectó la cuenta de Microsoft." },
  oautherror: {
    tone: "critico",
    text: "No se pudo conectar con Microsoft. Revisa los datos de Azure e inténtalo de nuevo.",
  },
  oauthfalta: {
    tone: "aviso",
    text: "Primero guarda el Tenant, Client ID y secreto de Azure; luego conecta.",
  },
};

// Translates a raw Microsoft 365 error into a concrete cause and its fix.
function errorHint(detail: string): string | null {
  ...
  if (
    ...
    d.includes("authentication unsuccessful") ||
...
export default async function EmailConfig({
  ...
  searchParams: Promise<{ guardado?: string; prueba?: string; detail?: string }>;
}) {
  const { guardado, prueba, detail } = await searchParams;
    ...
    <div className="pagina-head">
      <div>
        ...
        <p className="pagina-desc">
          Notificaciones al solicitante y avisos internos: método de envío, plantillas y
          preferencias
        </p>
      </div>
    </div>
  ...
  const operativo = isEmailReady(c);
  const conCreds = hasCredentials(c);
  const metodo = c?.metodo ?? "smtp_basico";
  const avisoPrueba = prueba ? AVISOS_PRUEBA[prueba] : null;
  const pista =
    (prueba === "error" || prueba === "oautherror") && detail ? errorHint(detail) : null;

  // Redirect URL that has to be registered in Azure for the interactive login.
  ...
  // Status chips on the collapsible headers (the structure conveys the state).
  ...
  // Service state: on and credentialed / off / incomplete.
  const estado = operativo
    ? {
        tone: "ok",
        titulo: "Activo",
        desc: "Las notificaciones se pueden enviar desde cada ticket.",
      }
    : conCreds
      ? {
          tone: "aviso",
          titulo: "Apagado",
          desc: "Hay credenciales, pero el interruptor está apagado: no se enviará nada.",
        }
      : {
          tone: "critico",
          titulo: "Sin configurar",
          desc: "Falta completar las credenciales del método elegido.",
        };
  ...
  return (
    <div className="correo-wrap">
      ...
      {guardado && (
        <div className="banner-config ok" role="status">
          Cambios guardados.
        </div>
      )}
      {avisoPrueba && (
        <div className={`banner-config ${avisoPrueba.tone}`} role="status">
          <div>{avisoPrueba.text}</div>
          {pista && <div className="banner-config-pista">{pista}</div>}
          {detail && (prueba === "error" || prueba === "oautherror") && (
      ...
      <div className={`estado-servicio ${estado.tone}`}>
      ...
      <form className="config-prueba destacada" action={sendTestMessage}>
        ...
        <div className="config-prueba-fila">
          <input
            ...
            aria-label="Correo de destino"
          />
          <SubmitButton className="boton" ocupado="Enviando…">
            Enviar prueba
          </SubmitButton>
        </div>
        <p className="config-ayuda" style={{ marginBottom: 0, marginTop: 10 }}>
          Usa el método y las credenciales guardadas. Si acabas de cambiar algo, guarda primero
          abajo.
        </p>
      </form>

      <form className="formulario config-form" action={saveEmailConfig}>
        {/* ---------- Método y conexión ---------- */}
        <details className="config-fold" open={!conCreds}>
          ...
          <div className="config-fold-cuerpo">
            ...
            <div className="metodo-opciones">
              <label className="metodo-opcion">
                <input
                  ...
                  name="metodo"
                  value="graph_app"
                  defaultChecked={metodo === "graph_app"}
                />
                <span className="metodo-cuerpo">
                  <span className="metodo-titulo">
                    Conexión de app (Graph) <span className="metodo-tag">recomendado</span>
                  </span>
                  <span className="metodo-detalle">
                    Sin contraseña, no caduca. Manda como un buzón fijo. Requiere registrar una app
                    en Azure.
                  </span>
                </span>
              </label>
              <label className="metodo-opcion">
                <input
                  ...
                  id="m-oauth"
                  name="metodo"
                  value="oauth_interactivo"
                  defaultChecked={metodo === "oauth_interactivo"}
                />
                <span className="metodo-cuerpo">
                  <span className="metodo-titulo">Iniciar sesión con Microsoft</span>
                  <span className="metodo-detalle">
                    Te lleva a la página de Microsoft, inicias sesión y manda como esa cuenta.
                  </span>
                </span>
              </label>
              <label className="metodo-opcion">
                <input
                  ...
                  name="metodo"
                  value="smtp_basico"
                  defaultChecked={metodo === "smtp_basico"}
                />
                <span className="metodo-cuerpo">
                  <span className="metodo-titulo">
                    SMTP básico <span className="metodo-tag legado">legado</span>
                  </span>
                  <span className="metodo-detalle">
                    Usuario y contraseña. Funciona hasta diciembre 2026.
                  </span>
                </span>
              </label>
            </div>
            ...
            <div className="metodo-bloque bloque-smtp">
              ...
              <p className="config-ayuda">
                Microsoft 365: <code>smtp.office365.com</code>, puerto <code>587</code>. El buzón
                necesita
                <strong> SMTP AUTH habilitado</strong> y, si tiene MFA, una{" "}
              ...
              <div className="campos">
                <div className="campo">
                  ...
                  <input
                    id="smtp_host"
                    name="smtp_host"
                    defaultValue={c?.smtp_host ?? "smtp.office365.com"}
                  />
                </div>
                <div className="campo">
                ...
                <div className="campo">
                  ...
                  <input
                    ...
                    name="smtp_user"
                    type="email"
                    defaultValue={c?.smtp_user ?? ""}
                    placeholder="soporteti@plasticospimsa.com"
                  />
                </div>
                <div className="campo">
                  ...
                  <input
                    ...
                    type="password"
                    placeholder={
                      c?.smtp_pass ? "•••••••• (sin cambios)" : "Contraseña o app password"
                    }
            ...
            <div className="metodo-bloque bloque-azure">
              ...
              <p className="config-ayuda">
                En{" "}
                <strong>
                  portal.azure.com → Microsoft Entra ID → Registros de aplicaciones → Nuevo registro
                </strong>
                , registra una app y copia aquí el <strong>Directorio (tenant) ID</strong> y el{" "}
                <strong>Id. de aplicación (cliente)</strong>. En{" "}
                <strong>Certificados y secretos</strong> genera un{" "}
                <strong>secreto de cliente</strong>.
              </p>
              <ul className="config-pasos">
                <li>
                  <strong>App-only (Graph):</strong> en Permisos de API agrega{" "}
                  <code>Microsoft Graph → Aplicación → Mail.Send</code> y da{" "}
                  <strong>consentimiento de administrador</strong>. Limita el envío al buzón con una{" "}
                  <code>ApplicationAccessPolicy</code>.
                </li>
                <li>
                  <strong>Login interactivo:</strong> agrega{" "}
                  <code>Microsoft Graph → Delegado → Mail.Send</code> y registra esta URL de
                  redirección (tipo Web):
                </li>
              </ul>
              ...
              <div className="campos">
                <div className="campo ancho">
                  ...
                  <input
                    ...
                    defaultValue={c?.azure_tenant_id ?? ""}
                ...
                <div className="campo">
                  ...
                  <input
                    ...
                    defaultValue={c?.azure_client_id ?? ""}
                    placeholder="00000000-0000-0000-0000-000000000000"
                  />
                </div>
                <div className="campo">
                  ...
                  <input
                    ...
                    type="password"
                    placeholder={
                      c?.azure_client_secret ? "•••••••• (sin cambios)" : "Valor del secreto"
                    }
                    autoComplete="new-password"
                  />
                </div>
        ...
        <details className="config-fold">
          <summary>
            ...
            <span className={`fold-chip ${avisoNuevoOn ? "ok" : "aviso"}`}>
              {avisoNuevoOn
                ? `${numDestinos} destinatario${numDestinos === 1 ? "" : "s"}`
                : "apagado"}
            </span>
          </summary>
          <div className="config-fold-cuerpo">
            <p className="config-ayuda">
              Cuando un empleado levanta un reporte en el portal, TI recibe un correo con el resumen
              y un botón para abrir el ticket. El envío usa el método configurado arriba (necesita
              el servicio activo).
            </p>
            <label className="config-switch">
              <input type="checkbox" name="notif_nuevo" defaultChecked={c?.notif_nuevo ?? true} />
              <span>
                <strong>Avisar por correo al entrar un ticket nuevo.</strong>
              </span>
            </label>
            <div className="campo ancho">
              ...
              <span className="campo-pista">
                Uno o varios correos, separados por coma o salto de línea.
              </span>
            </div>
            <div className="campo">
            ...
            <div className="campo">
              ...
              <textarea
                ...
                name="cuerpo_nuevo"
                rows={6}
                defaultValue={c?.cuerpo_nuevo ?? ""}
              />
            </div>
            <p className="config-ayuda" style={{ marginBottom: 0 }}>
              Variables: <code>{"{{folio}}"}</code> <code>{"{{titulo}}"}</code>{" "}
              <code>{"{{solicitante}}"}</code>
              <code>{"{{categoria}}"}</code> <code>{"{{descripcion}}"}</code>. El botón al ticket se
              agrega solo.
            </p>
          </div>
        </details>
        ...
        <details className="config-fold">
          <summary>
            Remitente y enlace
            <span className={`fold-chip ${remitente ? "" : "aviso"}`}>
              {remitente || "sin definir"}
            </span>
          </summary>
          <div className="config-fold-cuerpo">
            <div className="campos">
              ...
              <div className="campo">
                ...
                <input
                  ...
                  defaultValue={c?.remitente_nombre ?? "Soporte TI · Plásticos PIMSA"}
                />
              </div>
              <div className="campo ancho">
                ...
                <input
                  ...
                  name="sitio_url"
                  type="url"
                  defaultValue={c?.sitio_url ?? ""}
                  placeholder="https://ti-hub.vercel.app/"
                />
              </div>
            </div>
            <p className="config-ayuda" style={{ marginBottom: 0 }}>
              En el login interactivo se manda como la cuenta conectada; en los demás métodos, desde
              este buzón. La URL del portal arma el botón que abre el ticket o los reportes.
            </p>
          </div>
        </details>
        ...
        <details className="config-fold">
          ...
          <div className="config-fold-cuerpo">
            <p className="config-ayuda">
              ...
              <code>{"{{mensaje}}"}</code> (solo respuesta) <code>{"{{estado}}"}</code> (solo cambio
              de estado). El cuerpo es texto; se le aplica el diseño de marca PIMSA automáticamente.
            </p>
            ...
            <div className="campo">
              ...
              <input
                ...
                defaultValue={c?.asunto_respuesta ?? ""}
              />
            </div>
            <div className="campo">
              ...
              <textarea
                ...
                rows={6}
                defaultValue={c?.cuerpo_respuesta ?? ""}
              />
            </div>
        ...
        <details className="config-fold">
          <summary>
            Servicio y preferencias
            <span className={`fold-chip ${c?.activo ? "ok" : "aviso"}`}>
              {c?.activo ? "encendido" : "apagado"}
            </span>
          </summary>
          <div className="config-fold-cuerpo">
            <label className="config-switch">
              ...
              <span>
                <strong>Activar el envío de correos.</strong> Si está apagado, nunca se envía nada
                (ni avisos ni notificaciones).
              </span>
            </label>
            <label className="config-switch">
              <input
                type="checkbox"
                name="notif_respuesta_def"
                defaultChecked={c?.notif_respuesta_def ?? true}
              />
              <span>
                Al responder a un cliente, <strong>precargar</strong> la casilla “enviar por
                correo”.
              </span>
            </label>
            <label className="config-switch">
              <input
                type="checkbox"
                name="notif_estado_def"
                defaultChecked={c?.notif_estado_def ?? false}
              />
              <span>
                Al cambiar el estado, <strong>precargar</strong> la casilla “notificar por correo”.
              </span>
            </label>
          </div>
        </details>
        ...
        <details className="config-fold">
          ...
          <div className="config-fold-cuerpo">
            <p className="config-ayuda">
              Tiempos máximos comprometidos por prioridad, en <strong>horas de reloj</strong> (no
              horas hábiles). Vacío = usar el predeterminado de la app. Los valores predeterminados
              siguen ITIL 4 adaptado a empresa manufacturera con turnos continuos: crítica activa
              producción, alta bloquea a un usuario, media degrada parcialmente, baja es consulta o
              mejora.
            </p>
                ...
                [
                  {
                    ...
                    defReso: 4,
                    ref: "≤ 30 min / 4 h",
                    value: c?.sla_critica_respuesta,
                    valorR: c?.sla_critica_resolucion,
                  },
                  {
                    key: "alta",
                    label: "Alta",
                    ...
                    defReso: 24,
                    ref: "1–2 h / 4–8 h",
                    value: c?.sla_alta_respuesta,
                    valorR: c?.sla_alta_resolucion,
                  },
                  {
                    key: "media",
                    label: "Media",
                    ...
                    defReso: 48,
                    ref: "4–8 h / 24–48 h",
                    value: c?.sla_media_respuesta,
                    valorR: c?.sla_media_resolucion,
                  },
                  {
                    key: "baja",
                    label: "Baja",
                    ...
                    defReso: 96,
                    ref: "24 h / 72–120 h",
                    value: c?.sla_baja_respuesta,
                    valorR: c?.sla_baja_resolucion,
                  },
                ] as const
              ).map((fila) => (
                <div key={fila.key} className="sla-fila">
                  <span
                    className={`insignia ${{ critica: "critico", alta: "aviso", media: "info", baja: "neutro" }[fila.key]}`}
                  >
                    {fila.label}
                  </span>
                  <input
            ...
            <div className="campo" style={{ maxWidth: 260, marginTop: 16 }}>
              ...
              />
              <span className="campo-pista">
                Cuando el tiempo transcurrido supera este porcentaje del objetivo, el semáforo
                cambia de verde a naranja. Predeterminado: 80 %.
              </span>
            </div>
          </div>
        </details>
      ...
      <section className="config-seccion metodo-bloque bloque-oauth seccion-oauth">
        ...
        {conectado ? (
          <>
            ...
            <div className="oauth-acciones">
              <form action={connectMicrosoft}>
                <SubmitButton className="boton secundario" ocupado="Conectando…">
                  Reconectar
                </SubmitButton>
              </form>
          ...
          <>
            ...
            <form action={connectMicrosoft}>
              <SubmitButton className="boton" ocupado="Conectando…">

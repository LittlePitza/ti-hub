    ...
    <div className="pagina-head">
      <div>
        ...
        <p className="pagina-desc">
          Edita las cláusulas, accesorios y firmas de cada formato. Aplica a las responsivas que
          generes después.
        </p>
      </div>
    </div>
  ...
  return (
    <>
      ...
      <div className="plantillas-lista">
        {TEMPLATE_LIST.map((clave) => {
          const override = porClave.get(clave);
          const pl: Template = mergeTemplate(clave, override);
          const camposVisibles = new Set<string>(pl.camposEquipo ?? ALL_DEVICE_FIELDS);
            ...
            <details key={clave} className="plantilla-fila">
              <summary>
                ...
                <span className="suave mono" style={{ fontSize: 12.5 }}>
                  {pl.codigo} · v{override?.version ?? "1.0"}
                </span>
                {override && <span className="insignia info">editada</span>}
              </summary>

              <form className="formulario" action={saveTemplate} style={{ marginTop: 14 }}>
                ...
                <div className="campos">
                  ...
                  <div className="campo">
                    ...
                    <input
                      id={`${clave}-version`}
                      name="version"
                      defaultValue={override?.version ?? "1.0"}
                    />
                  </div>
                  ...
                  <div className="campo ancho">
                    ...
                    <textarea
                      id={`${clave}-accesorios`}
                      name="accesorios"
                      defaultValue={pl.accesorios.join("\n")}
                    />
                  </div>
                  <div className="campo ancho">
                    <label htmlFor={`${clave}-seguridad`}>
                      Configuración / verificación (una por línea)
                    </label>
                    <textarea
                      id={`${clave}-seguridad`}
                      name="seguridad"
                      defaultValue={pl.seguridad.join("\n")}
                    />
                  </div>
                  <fieldset className="grupo-chequeos campo ancho">
                    <legend>Datos del equipo en el documento</legend>
                    <div className="chequeos">
                      {CUSTODY_DEVICE_FIELDS.map((campo) => (
                        <label key={campo.clave} className="chequeo">
                          <input
                            type="checkbox"
                            name="campos_equipo"
                            value={campo.clave}
                            defaultChecked={camposVisibles.has(campo.clave)}
                          />
                    ...
                    <p className="suave" style={{ fontSize: 12, margin: "6px 2px 0" }}>
                      Algunos campos solo aparecen si el equipo los tiene (p. ej. teléfono solo en
                      celulares y líneas).
                    </p>
                  </fieldset>
                  <div className="campo ancho">
                    <label htmlFor={`${clave}-clausulas`}>
                      Cláusulas · formato «Título | texto», una por línea
                    </label>
                    <textarea
                      id={`${clave}-clausulas`}
                      name="clausulas"
                      defaultValue={clausulasTexto}
                      style={{ minHeight: 140 }}
                    />
                  </div>
                  <div className="campo ancho">
                    <label htmlFor={`${clave}-firmas`}>
                      Firmas · formato «Encabezado | nota», una por línea
                    </label>
              ...
              {override && (
                <form action={resetTemplate} style={{ marginTop: 8 }}>
                  <input type="hidden" name="clave" value={clave} />
                  <SubmitButton

...
*Superseded:* both faces now use **Poppins**, matching the PIMSA Portal de Mantenimiento. The
...
- [ ] 🟢 Notify IT when a new ticket arrives (email or Teams via webhook / Power Automate).
  *Partially done:* email notification on new ticket is implemented and configured from `/ti/correo`.
...
**Decision made — portal authentication:** *email only, no password and no magic link.*
...
- [x] 🟡 **`lib/domain/reports.ts`**: everything is **derived** from existing tables (no new tables,
  ...
  Standard help-desk cohorts: *created* (`created_at` in the month), *responded*
  (`primera_respuesta_at` in the month, giving the response SLA), *resolved* (`resuelto_at` in the
  month, giving the resolution SLA) and *backlog at close* (for the current month the cut is "now").
